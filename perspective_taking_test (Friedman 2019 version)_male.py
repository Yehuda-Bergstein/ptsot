...
TASK_Example_0 = ".םאתהב עיפוי וקו ןומיסה לגעמ ףקיה לע ץחל ,הבושתה ןומיס לע ןמאתהל ידכ .רבכעה תרזעב ךלש הבושתה ןומיס לע ןמאתה .הנוכנה הבושתה תנמוסמ וב המגודל ליגרת ךינפל\n" + \
                 ".תנמוסמה הנוכנה הבושתל ךלש הבושתה תא םאתה .היוצרה הבושתל רבכעה תרזעב ןומיסה לגעמ ךרואל וקה תא תא זזה\n\n" + \
                 ".םייסתשכ חוורה שקמ לע ץחל\n"

# this is the text to be shown alone, before the three practice examples
TASK_Example_1 = ".ךלש הבושתה תא ןמסל ידכ ןומיסה לגעמ לע וקה תא זזה אמגוד לכב .ןומיאל תואמגוד שולש ךינפל ועיפוי תעכ\n" +\
...
TASK_Example_2 = ".םייסתשכ חוורה שקמ לע ץחל\n"

# this is the text to be shown alone, before the 12 test trials
TASK_Example_3 = ".תחא המישמ לע ןמז ידמ רתוי זבזבת לא לבא ,קיודמב תונעל הסנ .תומישמ 21 םילשהל תוקד 5 ךתושרל .קדבמה ליחתי תעכ\n\n" +\
".וישכע לאש קדבמה יבגל תונורחא תולאש ךל שי םא\n\n" +\
".ליחתהל ןכומ התאש קדובל עדוה ,אל םא\n"

# this is the text to be shown below each of the test trials 
# below the TASK_TEXT_1, TASK_TEXT_2, and TASK_TEXT_3 combination
TASK_Example_4 = ".םייסתשכ חוורה שקמ לע ץחל\n"

# this is the text to be shown at the end of the test 
TASK_Example_5 = ".קדובל ארק אנא ,המייתסה המישמה\n"


TASK_TEXT_1 = "םוקמב דמוע התאש ןיימד"
TASK_TEXT_2 = "ןוויכל הנופו"
TASK_TEXT_3 = "ןוויכל עבצה"
...
# First 4 are example items, the next 12 are the actual test items - HEBREW
TASK_ITEMS = [
    ("ןומעפה", "ץעה", "ףותה", 306),
...
INSTRUCTION_TEXT = ".בחרמב תונוש טבמ תודוקנו םינוויכ ןיימדל ךלש תלוכיה תא ןחוב הז קדבמ\n" + \
                   ".ןומיס לגעמו טפשמ עיפוי הנומתל תחתמ .םיטקייבוא רפסמ םימקוממ הבו הנומת הארת קדבמב\n" + \
                   ".רחא טקייבוא ןוויכל הנופו ,הנומתבש םיטקייבואה דחא םוקמב דמוע התאש ןיימדל שקבתת התא\n" + \
                   ".תראותמה הביטקפסרפהמ ישילש טקייבוא אצמנ וב ןוויכה תא ףקשמש וק רייצל היהת ךלש המישמה\n" + \
                   ",שדח טקייבוא ןוויכל הנופו הנומתב רחא טקייבוא םוקמב דמוע התאש ןיימדל שקבתת בלש לכב\n" + \
                   ...
                   ".)הנופ התא וילא ינשה ץפחה לש ןוויכה( ךלש תניימודמה טבמה תדוקנ תא ףקשמ יכנאה ץחהו\n" + \
                   ".וללה תודוקנה יתשל סחיב ישילשה ץפחה לש ןוויכה תא גציימש וק רייצל ךירצ התא\n\n" + \
                   ".דומעה תיתחתבש אמגודל בלשב טבה\n" + \
                   ".ץעה ןוויכל הנופו ןומעפה םוקמב דמוע התאש ןיימדל שקבתמ התא וז אמגודב\n" + \
                   ".וקווקמ וקכ אמגודב עיפומ רייצל ךירצ היהש וקה .ףותה לש ןוויכה לא עיבצמש וק רייצל התייה אמגודב המישמה\n" + \
                   "?וקווקמה וקה עיבצמ וילא ןוויכב היה ףותה ,ץעה ןוויכל הנופו ןומעפה םוקמב דמוע תייה םאש ןיימדל לוכי התא םאה\nֿ\n" + \
                   ".המישמה יבגל תולאש ךל שי םא ןחובה תא לאש\n\n" + \
                   ".ךשמהל חוורה שקמ לע ץחל\n"
